...
import getpass, hashlib, logging, os, pickle, sys
import sqlite3

memoize_db_name = '/tmp/memoize-' + getpass.getuser()
conn = sqlite3.connect(memoize_db_name)
...
class memoize(object):
    ...
    def __init__(self, func):
        self._func = func
        try:
            conn.execute("CREATE TABLE {fn} (key text primary key, value blob)".format(
                    fn=self._func.func_name))
        except sqlite3.OperationalError as e:
            if 'already exists' not in e.message:
                raise
            pass
        self.cur = conn.execute('SELECT value FROM {fn} WHERE key=?'.format(
                fn=self._func.func_name))
        logging.info('opened sqlite3 cache for %s (%s)' % (self._func.func_name,self._cache))
